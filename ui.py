...
def coords(point):
    ...
    return 45 + point[0] * 40, 45 + point[1] * 40


def leftup_corner(point):
    return 27 + point[0] * 40, 27 + point[1] * 40
...
class UI:
    ...
    def draw(self, point, color, size=18):
        ...
        ccolor='BLACK' if color else 'WHITE'
        ccolor = get_rbg(ccolor)
        pygame.draw.circle(self.screen, ccolor, coords(point), size, 0)
        pygame.display.update()
